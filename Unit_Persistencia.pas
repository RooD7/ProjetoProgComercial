...
Begin
  ...
  Begin
    ...
    if DM.qryAux['Emp_InscEstadual'] <> null then
      Result.InscEstadual := DM.qryAux['Emp_InscEstadual']
    else
      Result.InscEstadual := '';
    if DM.qryAux['Emp_UF'] <> null then
      Result.UF := DM.qryAux['Emp_UF']
    else
      Result.UF := '';
    if DM.qryAux['Emp_TelResp'] <> null then
      Result.TelResp := DM.qryAux['Emp_TelResp']
    else
      Result.TelResp := '';
    if DM.qryAux['Emp_NomeResp'] <> null then
      Result.NomeResp := DM.qryAux['Emp_NomeResp']
    else
      Result.NomeResp := '';
    if DM.qryAux['Emp_CargoResp'] <> null then
      Result.CargoResp := DM.qryAux['Emp_CargoResp']
    else
      Result.CargoResp := 0;
