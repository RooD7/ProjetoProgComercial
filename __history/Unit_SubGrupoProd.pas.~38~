...
Begin
  ...
  if Not(btnAZ.Enabled) then
    SubGruposP_Atuais := Retorna_SubGrupoProds_Cadastrados(Campo, Termo, 'AZ');
  if Not(btnZA.Enabled) then
    SubGruposP_Atuais := Retorna_SubGrupoProds_Cadastrados(Campo, Termo, 'ZA');
  if Not(btn09.Enabled) then
    SubGruposP_Atuais := Retorna_SubGrupoProds_Cadastrados(Campo, Termo, '09');
  if Not(btn90.Enabled) then
    SubGruposP_Atuais := Retorna_SubGrupoProds_Cadastrados(Campo, Termo, '90');
  If Length(SubGruposP_Atuais) >= 1 Then
    sgdSubGrupoProd.RowCount := Length(SubGruposP_Atuais) + 1
  Else
  Begin
  ...
  End;
  lblQtde.Caption := IntToStr(Length(SubGruposP_Atuais)) +
    ' subgrupos de produtos cadastrados';
  ...
  Begin
    ...
    sgdSubGrupoProd.Cells[2, I + 1] := Retorna_Dados_GrupoProd
      (SubGruposP_Atuais[I].Cod_Grupo).Descricao;
  End; // end do FOR
...
procedure TfrmSubGrupoProd.sgdSubGrupoProdSelectCell(Sender: TObject;
...
Var
  Prod_Com_Subgrupo: Prods_Com_SubGrupo;
Begin
  If Validado Then
  Begin
    If Novo Then
      Grava_Dados_SubGrupoProd(Coleta_Dados)
    Else
    Begin
      Prod_Com_Subgrupo := Tem_Prod_SubGrupo(Coleta_Dados.Codigo);
      if Length(Prod_Com_Subgrupo) > 0 then
        if Application.MessageBox
          ('Prezado usuário, existem produtos cadastrados neste subgrupo. Deseja alterá-los automaticamente para o novo subgrupo?',
          'Atualizar os subgrupos de todos os produtos?',
          MB_ICONWARNING + MB_YESNOCANCEL) = mrYes then
        Begin
          Atualiza_Produtos_Cascata(Prod_Com_Subgrupo,
            Retorna_Codigo_Grupo(cbxGrupo.Text));
        End;
