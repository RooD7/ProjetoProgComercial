...
procedure TfrmClientes.Popula_Grid;
  ...
  Begin
    ...
    if Not(btnAZ.Enabled)
      then Clientes_Atuais := Retorna_Clientes_Cadastrados(Campo, Termo, 'AZ');
    if Not(btnZA.Enabled)
      then Clientes_Atuais := Retorna_Clientes_Cadastrados(Campo, Termo, 'ZA');
    if Not(btn09.Enabled)
      then Clientes_Atuais := Retorna_Clientes_Cadastrados(Campo, Termo, '09');
    if Not(btn90.Enabled)
      then Clientes_Atuais := Retorna_Clientes_Cadastrados(Campo, Termo, '90');
    If Length(Clientes_Atuais) >= 1
      ...
      Else Begin
           ...
           End;
    lblQtde.Caption := IntToStr(Length(Clientes_Atuais))+ ' clientes cadastrados';
...
procedure TfrmClientes.sgdClientesSelectCell(Sender: TObject; ACol,
...
Application.MessageBox('O campo de endereço é obrigatório',
    ...
    if Not(isCPF(Temp_CPF))
      Then Begin
Application.MessageBox('O CPF é inválido. Digite o CPF correto e tente novamente!',
                       'CPF inválido',
                       MB_ICONERROR+MB_OK);
             Result := False;
             PageControl1.ActivePageIndex := 1;
